...
The system covers two parallel data flows: streaming user interactions from the web app into Cassandra (via Avro schemas), and Change Data Capture (CDC) from the transactional database into a MinIO data lake (via Debezium).
...
Each component lives in its own repository, with detailed setup instructions in its respective README:
